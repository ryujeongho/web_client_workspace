...
/**
 * Web Storage
 * - 웹브라우져에 key/value 형식으로 데이터를 보관하는 기술
 ...
 * - Session Storage - 서버접속한 동안 보관(윈도우를 닫으면 제거)
 * 
 */
...
document.querySelector("#btn4").onclick = () => localStorage.clear(); // 전체삭제
...
document.querySelector("#btn5").onclick = () => {
    ...
    console.log(obj2, typeof obj2); // {username: 'honggd', id: 12341235, married: true} 'object'

    const arr = ['a', 'b', 'c'];
    localStorage.setItem('arr', JSON.stringify(arr));

    const arr2 = JSON.parse(localStorage.getItem('arr'));
    console.log(arr2, typeof arr2); // (3) ['a', 'b', 'c']0: "a"1: "b"2: "c"length: 3[[Prototype]]: Array(0) 'object'
...
const saveGuestbook = () => {
    ...
    const guestbooks = JSON.parse(localStorage.getItem('guestbooks')) || []; // 앞에가 false면 뒤에 값을 대입
