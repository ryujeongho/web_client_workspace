// test1();
// 함수 선언식에는 정상 작동
// 함수 표현식에는 오류 Uncaught ReferenceError: Cannot access 'test1' before initialization
/**
 * 함수 선언식
 * - hoisting 처리됨.
...
//     return a + b + c;
// };
// return 구문 한 줄만 있을 때 { return } 생략가능
...
//     switch (type) {
//         case 'plus':
//             result = numbers.reduce((agg, n) => {
//             return agg + n;
//         }); break;
//         case 'minus': 
//         result = numbers.reduce((agg, n) => agg - n); break;
//         case 'multiply': 
//         result = numbers.reduce((agg, n) => agg * n); break;
//         case 'divide': 
//         result = numbers.reduce((agg, n) => agg / n); break;
...
const test8 = () => {
    ...
    sayGoodbye('길동');
    // 돈 좀 꿔줘~ 길동
    const giveMoney = getFunction2('돈 좀 꿔줘');
    giveMoney('길동');
};
...
//     return (name) => console.log(`${greeting}~ ${name}`); // Hello~ 길동
// }
